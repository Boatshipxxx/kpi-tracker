...
function parseWebhookBody(body) {
  ...
  return {
    ...
    endDateTime: typeof p.endDateTime === 'string' && p.endDateTime ? p.endDateTime : null,
    timeZone: typeof p.timeZone === 'string' && p.timeZone ? p.timeZone : 'Asia/Tokyo',
    company: answerOf(COMPANY_QUESTION),
    // Web会議情報。予約確定メールに載せるために使う。
    // Spir 側で会議ツールが未連携だと URL が空で届くことがあるため、
    // 呼び出し側は url が無い前提でも動くようにすること。
    onlineMeeting: p.onlineMeeting && typeof p.onlineMeeting === 'object'
      ? {
        type: typeof p.onlineMeeting.type === 'string' ? p.onlineMeeting.type : null,
        url: typeof p.onlineMeeting.url === 'string' && p.onlineMeeting.url.trim() ? p.onlineMeeting.url.trim() : null,
        password: typeof p.onlineMeeting.password === 'string' && p.onlineMeeting.password.trim() ? p.onlineMeeting.password.trim() : null,
      }
      : { type: null, url: null, password: null },
  };
}
