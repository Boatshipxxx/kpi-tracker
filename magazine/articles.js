const ARTICLES = [
  {
    ...
    body: `<p>Shake Shack Japanの公式ウェブサイトが新しくなりました。BOATshipはUI設計からフロントエンド実装まで、プロジェクト全体を担当しました。</p>
<h2>プロジェクト概要</h2>
<p>Shake Shackは、ニューヨーク発のバーガーブランド。日本上陸以来、ファンを増やし続けています。今回のリニューアルでは、ブランドのプレミアム感を損なわずに、ユーザビリティを大きく向上させることが求められました。</p>
<h2>実装した機能</h2>
...
<p>HTML / CSS / JavaScript のピュアな実装。フレームワークを使わずに、軽量で高速なサイトを実現しました。表示速度はリニューアル前比で40%改善。Core Web Vitalsのすべての指標でGreenを達成しています。</p>`
  },
  {
    ...
    excerpt: 'nahrinプロジェクトで得たShopify Liquidの実装ノウハウ。メタフィールド活用からカスタムセクション設計まで。',
    body: `<p>nahrinのECサイト構築で得た、Shopify Liquid実装のTipsをまとめます。Liquidの独特な記法に慣れると、WordPressよりも柔軟な実装が可能です。</p>
...
<p>テーマのカスタマイズはセクションとブロックで設計しましょう。schema JSONで編集可能な項目を定義することで、コードを触らずにページレイアウトを変更できます。クライアントが自分で運用できる仕組みが大切です。</p>
...
<p>metaobjectsを使うと、ブランド情報や店舗情報などをコンテンツとして管理できます。nahrinでは各商品のストーリーをmetaobjectで管理し、複数ページで再利用しています。</p>`
  },
  {
    id: '03',
    num: '#03',
...
<p>Balenciaga、Vogue Italia、Dazed Digital。これらのサイトは、ブランドの力でブルータリズムを成立させています。デザインに「理由」があるとき、荒削りは力になります。</p>`
  },
  {
    ...
    hero: '../images/works-nahrin.jpg',
    excerpt: 'スイス発ナチュラルコスメブランドnahrinの公式ECサイトがリニューアル。新機能と新デザインを公開。',
    body: `<p>スイス発のナチュラルコスメブランド「nahrin」の公式ECサイトがリニューアルしました。BOATshipはShopifyを使った設計・実装を担当しました。</p>
<h2>ブランドの世界観を忠実に</h2>
<p>nahrinは、スイスのアルプスで採取したハーブを原料にした自然派コスメブランドです。「清潔感」「自然」「信頼」というブランドの核心を、デジタルの文脈で表現することが最初の課題でした。</p>
<h2>新機能の概要</h2>
...
<p>日本のEC購入の70%以上がスマートフォン経由。モバイルでの購入導線を徹底的に最適化しました。商品ページのCTA配置、カート操作のステップ数を削減し、コンバージョン率の向上を目指しました。</p>`
  },
  {
    ...
    excerpt: 'EYEWEAR MEBIUSとTEAM JOSAI!の実装から得た、WordPressカスタムテーマ開発のベストプラクティス。',
    body: `<p>WordPressは「使いこなすのが難しい」と言われますが、カスタムテーマの設計を正しく行えば、クライアントが自走できる強力なCMSになります。</p>
<h2>ファイル構成の原則</h2>
...
<h2>カスタム投稿タイプの設計</h2>
<p>EYEWEAR MEBIUSでは、ブランドとプロダクトカテゴリをカスタム投稿タイプで管理しています。<code>register_post_type()</code>と<code>register_taxonomy()</code>を使い、管理画面をクライアントが直感的に操作できる構造にしました。</p>
<h2>ACF（Advanced Custom Fields）活用</h2>
<p>標準のWYSIWYGエディタでは表現できない複雑なレイアウトには、ACFのフレキシブルコンテンツが有効です。TEAM JOSAI!では、選手インタビュー、試合レポート、ギャラリーなど、記事の種類に応じたレイアウトをフレキシブルコンテンツで管理しています。</p>
<h2>パフォーマンス</h2>
